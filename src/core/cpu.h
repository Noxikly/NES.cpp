...
public:
    ...
    private:
        ...
        inline void set_flag(u8 flag, bool value) {
            (value) ? (regs.P |= flag) : (regs.P &= ~flag);
        }
        inline void set_nz(u8 value) {
            set_flag(Z, value == 0);
            set_flag(N, value & N);
        }
        inline void set_nz16(u16 value) {
            set_flag(Z, (value & 0x00FF) == 0);
    ...
    private:
        ...
        inline u16 AM_ZPX() {
            return (AM_ZPG() + static_cast<u16>(regs.X)) & 0x00FF;
        }
        inline u16 AM_ZPY() {
            return (AM_ZPG() + static_cast<u16>(regs.Y)) & 0x00FF;
        }
        ...
        inline u16 AM_INX() {
            const u16 addr = read16_zp(AM_ZPX());
            return addr;
        }
        inline u16 AM_INY() {
            const u16 addr = read16_zp(AM_ZPG());
            const u16 idx_addr = addr + regs.Y;
            page_crossed = (addr & 0xFF00) != (idx_addr & 0xFF00);
            return idx_addr;
        }
