...
/* STA: A -> M */
inline void CPU::C6502::STA(u16 addr) {
    p->mem->write(addr, regs.A);
}
...
/* STX: X -> M */
inline void CPU::C6502::STX(u16 addr) {
    p->mem->write(addr, regs.X);
}
...
/* STY: Y -> M */
inline void CPU::C6502::STY(u16 addr) {
    p->mem->write(addr, regs.Y);
}
...
inline void CPU::C6502::ArithmWithCarry(const u16 val) {
    const u16 tmpA = static_cast<u16>(regs.A) 
                   + val 
                   + static_cast<u16>(regs.P & C);
...
/* INX: X + 1 -> X */
inline void CPU::C6502::INX() {
    set_nz(++regs.X);
}

/* DEX: X - 1 -> X */
inline void CPU::C6502::DEX() {
    set_nz(--regs.X);
}

/* INY: Y + 1 -> Y */
inline void CPU::C6502::INY() {
    set_nz(++regs.Y);
}

/* DEY: Y - 1 -> Y */
inline void CPU::C6502::DEY() {
    set_nz(--regs.Y);
}
...
inline void CPU::C6502::ROR(u16 addr) {
    const bool cond = p->c.opEntry.am == C6502::IMP;
    const u8 val = (cond) ? regs.A : p->mem->read(addr);
    const u16 tmp = static_cast<u16>((regs.P & C) << 7) 
                    | (val >> 1);
...
inline void CPU::C6502::CMP(u16 addr) {
    const u8 val = p->mem->read(addr);
    const u16 tmp = static_cast<u16>(regs.A)
                    - static_cast<u16>(val);
...
inline void CPU::C6502::CPX(u16 addr) {
    const u8 val = p->mem->read(addr);
    const u16 tmp = static_cast<u16>(regs.X)
                    - static_cast<u16>(val);
...
inline void CPU::C6502::CPY(u16 addr) {
    const u8 val = p->mem->read(addr);
    const u16 tmp = static_cast<u16>(regs.Y)
                    - static_cast<u16>(val);
...
/* JMP: PC = M */
inline void CPU::C6502::JMP(u16 addr) {
    regs.PC = addr;
}
...
inline void CPU::C6502::RTS() {
    const u8 low = pop();
    const u8 high = pop();
    regs.PC = (static_cast<u16>(high) << 8) 
              | static_cast<u16>(low);
    regs.PC++;
}
...
/* SEC: Set Carry */
inline void CPU::C6502::SEC() {
    set_flag(C, 1);
}

/* CLC: Clear Carry */
inline void CPU::C6502::CLC() {
    set_flag(C, 0);
}

/* SEI: Set Interrupt */
inline void CPU::C6502::SEI() {
    set_flag(I, 1);
}

/* CLI: Clear Interrupt */
inline void CPU::C6502::CLI() {
    set_flag(I, 0);
}

/* SED: Set Decimal*/
inline void CPU::C6502::SED() {
    set_flag(D, 1);
}

/* CLD: Clear Decimal */
inline void CPU::C6502::CLD() {
    set_flag(D, 0);
}

/* CLV: Clear Overflow */
inline void CPU::C6502::CLV() {
    set_flag(V, 0);
}


/* NOP: ничего не делать */
inline void CPU::C6502::NOP() {
    /* Ничего */
}
...
inline void CPU::C6502::USBC(u16 addr) {
    SBC(addr);
}

inline void CPU::C6502::KIL(u16 /*addr*/) {
    regs.PC--;
}

inline void CPU::C6502::NOP_IGN(u16 addr) {
    (void)p->mem->read(addr);
}
